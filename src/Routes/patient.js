...
patient_api.post('/create', async (req, res) => {
  ...
  if (!Patient_present) {
    const create = await Patient.create(formValues).catch((error) => {
      return res.status(500).json({ error: "Not Found", message: "something went wrong pls check filed" })
    })
  ...
  return res.status(500).json({ error: err, message: "something went wrong pls check filed" })
} 
});
